...
class RV_Adapter_Community_List() :
    RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    var items: ArrayList<CommunityDao>? = null
    var photoItem: ArrayList<PhotoDao>? = null
    var fragmentManager: FragmentManager? = null
    var arrList = ArrayList<CommunityDao>()
    lateinit var inflater: LayoutInflater
//    var listField : Array<String> = inflater.context.resources.getStringArray(R.array.information)

    constructor(items: ArrayList<CommunityDao>, photoItem: ArrayList<PhotoDao>, fragmentManager: FragmentManager) : this() {
        this.items = items
...
//            .into(vh1.iv!!)
        ...
        var temp: List<PhotoDao> = photoItem!!.filter { it.data.item_id.equals(items!!.get(position).id) }
        if (temp.size != 0){
            Glide.with(inflater.context)
